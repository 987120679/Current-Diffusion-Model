...
def sample(args):
    ...
    for _ in pbar:
        ...
        for y_mag, f, current_subfolders in loader:
            ...
            samples_out = vae.decode_quant_VDT(samples)
            samples_out = rearrange(samples_out, '(b f) c h w -> b f c h w', b=B)
            samples = rearrange(samples, '(b f) c h w -> b f c h w', b=B)
            B, T, c, H, W = samples_out.shape
            # print(samples.shape)
            ...
            for i, samples_out in enumerate(samples_out):
                # index = i * dist.get_world_size() + rank + total
                os.makedirs(f"{sample_folder_dir}/{current_subfolders[i]}", exist_ok=True)
                f=h5py.File(f"{sample_folder_dir}/{current_subfolders[i]}/surfj.hdf5","w")
                surfj_data = samples_out.permute(1,0,2,3).cpu().numpy()
                f.create_dataset("sufj",data=surfj_data)
                f.close()
                for cc in range(c):
                    samples_out_cc = samples_out[:,cc,:,:]
                    samples_out_cc = samples_out_cc.contiguous().view(-1, 1, H, W).to(device=device)
                    save_image(samples_out_cc, f"{sample_folder_dir}/{current_subfolders[i]}/surface_channel{cc:06d}.png", nrow=args.num_frames, normalize=True, value_range=(0, 1.5))
            # also save latent samples for review
            for j, samples in enumerate(samples):
                os.makedirs(f"{sample_folder_dir}/{current_subfolders[j]}", exist_ok=True)
                # index = j * dist.get_world_size() + rank + total
                for CC in range(C):
                    samples_cc = samples[:,CC,:,:]
                    samples_cc = samples_cc.contiguous().view(-1, 1, h, w).to(device=device)
                    save_image(samples_cc, f"{sample_folder_dir}/{current_subfolders[j]}/latent_channel{CC:06d}.png", nrow=args.num_frames, normalize=True, value_range=(-1.3, 1.3))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--per-proc-batch-size", type=int, default=7)
    parser.add_argument("--num-workers", type=int, default=1)
    parser.add_argument("--image-size", type=int, choices=[64, 128, 256, 512], default=128)
    parser.add_argument("--num-classes", type=int, default=1000)
    parser.add_argument('--device', default='cuda')
    parser.add_argument("--data-path", type=str, default='./vae/samples/ol/data_file')
    ...
    parser.add_argument("--ckpt", type=str, default="fullsize_cos_mag_tsqrt_smag_MVDT_605000.pt",
    ...
    parser.add_argument("--special-discription",  type=str, default="samples_partS")
    parser.add_argument("--num-fid-samples", type=int, default=7)
